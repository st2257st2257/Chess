...
def change_color(color):
    '''
    Takes color and returns opposite one
    '''
    if color == 'black':
        return 'white'
    else:
...
def game(id, username):
    ...
    party = Party()
    players_data = {
        'player': username, 'player_rating': str(cl.check_rate(username))}
    finished = False
    moves_font = pygame.font.SysFont('FreeSerif', int(80 * scale_x))
    button_font = pygame.font.SysFont('Arial', int(50 * scale_x))
    moves_vis = Scroll_window(
        0, 0, [], moves_font, int(desk_x_coord), int(450 * scale_y), False)
    surrender_button = button(
        int(50 * scale_x), int(500 * scale_y), 'Surrender', button_font)
    white = cl.get_white(id)
    if white == username:
        color = 'white'
        black = cl.get_black(id)
        players_data.update({
            'opponent': black, 'opponent_rating': str(cl.check_rate(black))})
    else:
        color = 'black'
        players_data.update({
            'opponent': white, 'opponent_rating': str(cl.check_rate(white))})
    while not finished:
            ...
            while not finished_1:
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        cl.update_party_figures(
                            id, figures_to_string(party.fields))
                        finished_1 = True
                        ...
                        if surrender_button.check():
                            ...
                            cl.add_move(id, change_color(color) + '_win')
                            cl.update_rate(
                                username, players_data['opponent'], 2)
                    moves_vis.event_handler(event)
                ...
                if frame_counter >= FPS:
                    ...
                    if finished_1 and not finished:
                        ...
                        if 'pawn' in party.fields[(
                                int(last_move[0]),
                                int(last_move[1]))].figuretype and abs(
                                int(last_move[3]) - int(last_move[1])) == 2:
                            party.last_pawn = party.fields[(
                                int(last_move[2]), int(last_move[3]))]
                        party.fields = string_to_figures(
                            cl.get_party_figures(id), party.fields)
    post_game_lobby(id, color)


def post_game_lobby(id, color):
    '''
    Function with watching game with *id* number from a perspective of
    *color* player.
    '''
    ...
    to_main_button = button(
        int(10 * scale_x), int(500 * scale_y),
        'Back to main menu', button_font)
    ...
    for move in moves[::-1]:
        ...
        if 'O-O' in move:
            if move[2] == '7':
                party.fields[8, int(move[1])].figuretype = party.fields[
                    6, int(move[1])].figuretype
                party.fields[6, int(move[1])].figuretype = 'empty'
            else:
                party.fields[1, int(move[1])].figuretype = party.fields[
                    4, int(move[1])].figuretype
                party.fields[4, int(move[1])].figuretype = 'empty'
            fig = 'empty'
        party.fields[int(move[0]), int(move[1])].figuretype = party.fields[
            int(move[2]), int(move[3])].figuretype
        party.fields[int(move[2]), int(move[3])].figuretype = fig
        states.insert(0, figures_to_string(party.fields))
    finished = False
    moves_visual = Scroll_window(0, 0, moves, moves_font, int(
        desk_x_coord), 450 * scale_y, True)
    while not finished:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                ...
                if to_main_button.check():
                    finished = True
            state_num = moves_visual.event_handler(event)
            if state_num is not None:
                party.fields = string_to_figures(
                    states[state_num + 1], party.fields)
